...
const TEMPLATES: Template[] = [
  {
    name: "最大力量日", icon: "🏋️", desc: "下肢力量为主，适合力量积累期",
    exercises: [
      { exerciseId: "back-squat", phase: "main", sets: 4, reps: "5", restSec: 180, note: "80-85% 1RM" },
      { exerciseId: "romanian-deadlift", phase: "main", sets: 3, reps: "8", restSec: 120, note: "控制离心" },
      ...
      { exerciseId: "foam-rolling-quads", phase: "cooldown", sets: 1, reps: "2分钟", restSec: 0, note: "" },
    ],
  },
  {
    name: "增强式爆发日", icon: "⚡", desc: "跳跃/增强式为主，适合爆发力转化期",
    exercises: [
      ...
      { exerciseId: "hurdle-hop", phase: "main", sets: 3, reps: "6栏", restSec: 120, note: "低栏(30cm)" },
      { exerciseId: "pigeon-stretch", phase: "cooldown", sets: 1, reps: "60s/侧", restSec: 0, note: "" },
    ],
  },
  {
    name: "奥林匹克举重日", icon: "🏆", desc: "举重衍生+力量，适合神经爆发训练",
    exercises: [
  ...
  {
    name: "速度敏捷日", icon: "🏃", desc: "短跑+绳梯+增强式组合",
    exercises: [
  ...
  {
    name: "恢复与预康复日", icon: "🧘", desc: "低强度恢复、活动度、预康复训练",
    exercises: [
  ...
  return (
    <div className={`bg-[#1e293b] border rounded-lg p-3 flex items-center gap-3 transition-all ${added ? "border-amber-500/40 opacity-70" : "border-slate-700/50 hover:border-slate-500"}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-500 truncate">{ex.targetMuscles}</p>
      </div>
      {!added ? (
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="text-center mb-8">
        <h1 className="text-3xl font-extrabold text-slate-50 mb-2">🧩 训练组装器</h1>
        <p className="text-slate-400">从动作库挑选动作，组装你的专属训练日计划</p>
      </div>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-[#1e293b] border border-slate-700/50 rounded-xl p-4">
            <h2 className="text-sm font-bold text-slate-200 mb-3 flex items-center gap-2">📚 动作库</h2>
            {/* Search */}
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              <button onClick={() => setActiveCategory("all")}
                className={`px-2.5 py-1 text-xs rounded-md transition-colors ${activeCategory === "all" ? "bg-amber-500/15 text-amber-400" : "bg-slate-800 text-slate-400 hover:text-slate-200"}`}>
                全部
              </button>
              {CATEGORIES.map(cat => (
                <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
                  className={`px-2.5 py-1 text-xs rounded-md transition-colors ${activeCategory === cat.id ? "bg-amber-500/15 text-amber-400" : "bg-slate-800 text-slate-400 hover:text-slate-200"}`}>
                  {cat.icon}
                </button>
              ))}
            </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-[#1e293b] border border-slate-700/50 rounded-xl p-4">
            <div className="flex items-center gap-3 flex-wrap">
              <input type="text" value={planName} onChange={e => setPlanName(e.target.value)}
                className="flex-1 min-w-[120px] bg-transparent text-lg font-bold text-slate-100 outline-none border-b border-transparent focus:border-amber-500/50 transition-colors px-1"
                placeholder="输入计划名称..." />
              <span className="text-xs text-slate-500 bg-slate-800 px-2 py-1 rounded">
                {plan.length} 动作 · 约 {Math.round(totalTimeMin)} 分钟
              </span>
              {/* Action buttons */}
              <button onClick={savePlan} className="px-3 py-1.5 text-xs bg-amber-500/15 text-amber-400 border border-amber-500/30 rounded-lg hover:bg-amber-500/25 transition-colors flex items-center gap-1">
                💾 保存
              </button>
              <button onClick={() => setShowSaved(true)} className="px-3 py-1.5 text-xs bg-slate-700 text-slate-300 border border-slate-600 rounded-lg hover:bg-slate-600 transition-colors flex items-center gap-1" disabled={savedPlans.length === 0}>
                📂 加载
              </button>
              <button onClick={() => setShowTemplates(true)} className="px-3 py-1.5 text-xs bg-slate-700 text-slate-300 border border-slate-600 rounded-lg hover:bg-slate-600 transition-colors flex items-center gap-1">
                📋 模板
              </button>
              <button onClick={exportPlan} className="px-3 py-1.5 text-xs bg-slate-700 text-slate-300 border border-slate-600 rounded-lg hover:bg-slate-600 transition-colors flex items-center gap-1" disabled={plan.length === 0}>
                📤 导出
              </button>
              <button onClick={() => { dispatch({ type: "CLEAR_PLAN" }); setPlanName("我的训练计划"); showToast("🗑️ 已清空"); }}
                className="px-3 py-1.5 text-xs bg-red-500/10 text-red-400 border border-red-500/20 rounded-lg hover:bg-red-500/20 transition-colors flex items-center gap-1" disabled={plan.length === 0}>
                ✕ 清空
              </button>
            </div>
          </div>
          ...
          {/* Plan content */}
          {plan.length === 0 ? (
            <div className="bg-[#1e293b] border border-dashed border-slate-600/50 rounded-xl p-12 text-center">
              <p className="text-5xl mb-4">🧩</p>
              <p className="text-slate-400 mb-2">你的训练计划还是空的</p>
              <p className="text-sm text-slate-500">从左侧动作库点击「+ 添加」开始组装<br />或使用「📋 模板」快速生成</p>
            </div>
          ) : (
            <div className="space-y-4">
              {groupedPlan.map(group => (
                <div key={group.phase} className="bg-[#1e293b] border border-slate-700/50 rounded-xl overflow-hidden">
                  <div className="px-4 py-3 border-b border-slate-700/30 flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500">· {group.exercises.length} 个动作</span>
                    <span className="text-xs text-slate-600 ml-auto">{group.desc}</span>
                  </div>
      ...
      {showTemplates && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 flex items-center justify-center p-4" onClick={() => setShowTemplates(false)}>
          <div className="bg-[#1e293b] border border-slate-600 rounded-2xl max-w-md w-full p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-slate-100 mb-4">📋 预设模板</h3>
            <div className="space-y-2 max-h-[50vh] overflow-y-auto">
              {TEMPLATES.map(t => (
                <button key={t.name} onClick={() => loadTemplate(t)}
                  className="w-full text-left p-3 bg-slate-800/50 border border-slate-700/50 rounded-lg hover:border-amber-500/40 transition-colors group">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xl">{t.icon}</span>
                    <span className="font-semibold text-slate-200 text-sm">{t.name}</span>
                    <span className="text-xs text-slate-500 ml-auto">{t.exercises.length} 动作</span>
                  </div>
