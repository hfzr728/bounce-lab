...
import { extractInjuryInfo, buildInjuryConstraintText, scanAndWarnPlan } from "@/lib/ai/injury-restrictions";
...
1. **安全第一**：绝不推荐用户明确表示有禁忌/伤病的动作。如果用户有伤病史，必须用安全的替代动作替换所有禁忌动作。
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 提取伤病信息并构建约束
    const injury = answers ? extractInjuryInfo(answers) : undefined;
...
- 输出完整的12周计划（不要偷懒省略后几周）`;
    ...
    const rawPlan = await callDeepSeek(REVISE_SYSTEM_PROMPT, userPrompt, {
      temperature: 0.7,
      maxTokens: 8192,
    });

    // 后处理：扫描违禁动作 + XSS 清洗
    const { text: warnedPlan, hasViolations, violations } = injury
      ? scanAndWarnPlan(rawPlan, injury)
      : { text: rawPlan, hasViolations: false, violations: [] };
    ...
    return NextResponse.json({
      ...
      safetyWarnings: hasViolations ? {
        violations,
        message: `系统检测到修订后的计划中仍包含 ${violations.length} 个不适宜你伤病史的动作，已在计划前追加安全警告。`,
      } : undefined,
    });
  } catch (error: unknown) {
